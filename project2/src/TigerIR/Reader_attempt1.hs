...
parseStartFunc = void (string "#start_function")
...
parseImm :: Parsec' Imm
parseImm = Imm <$> many1 (char '-' <|> digit)

parseVariable :: Parsec' Variable
parseVariable = p <?> "variable"
  where
    p = Variable <$> parseIdentifier

parseFunctionName :: Parsec' FunctionName
parseFunctionName = FunctionName <$> parseIdentifier

parseBinOperands :: Parsec' BinOperands
parseBinOperands = do
  parseVVV 